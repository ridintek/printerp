...
class Cron_model extends CI_Model
{
  ...
  {
    if ($total = $this->site->syncProducts()) {
      $this->rdlog->info(sprintf("%d products have been synced successfully.", $total));
    }
  ...
  private function resetOrderRef() // OK
  {
    if ($this->Settings->reference_format == 1 || $this->Settings->reference_format == 2) {
      ...
      if ($ref = $this->getOrderRef()) {
        $reset_ref = ['adjustment' => 1, 'cmreport' => 1, 'expense' => 1, 'income' => 1, 'iuse' => 1,
