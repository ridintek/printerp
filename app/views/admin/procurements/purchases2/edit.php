...
<div class="modal-content">
  <div class="modal-header">
    ...
    <h4 class="modal-title text-center" id="myModalLabel"><?= ucfirst($mode) ?> Purchase</h4>
  </div>
...
<script>
  $(function() {
    ...
    purchase.setMode('<?= $mode ?>'); // edit, status

    q += 'warehouse=' + $('#from_warehouse').val();

    if (pmitems) {
      for (let pmitem of pmitems) {
        purchase.addItem(pmitem);
      }
    }
    ...
    $('#submit').click(function() {
      ...
      $.ajax({
        ...
        url: site.base_url + 'procurements/purchases2/edit/<?= $pm->id ?>'
      })
    });
  });
</script>