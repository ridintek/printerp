...
if ($warehousesFrom = $this->input->get('warehouses_from')) {
  foreach ($warehousesFrom as $warehouseId) {
    if (!empty($warehouseId)) {
      $q .= '&warehouse_id_from[]=' . $warehouseId;
    }
  }
}

if ($warehousesTo = $this->input->get('warehouses_to')) {
  foreach ($warehousesTo as $warehouseId) {
    if (!empty($warehouseId)) {
      $q .= '&warehouse_id_to[]=' . $warehouseId;
    }
  }
}
          ...
          <div class="row">
            <div class="col-sm-4">
              <div class="form-group">
                <label for="warehousesFrom"><i class="fad fa-warehouse"></i> Warehouses From</label>
                <?php
                ...
                ?>
                <?= form_multiselect('warehouses_from[]', $bl, ($warehouses ?? ''), 'class="select2" id="warehousesFrom" data-placeholder="Select warehouse from" style="width:100%;"'); ?>
              </div>
            </div>
            <div class="col-sm-4">
              <div class="form-group">
                <label for="warehousesTo"><i class="fad fa-warehouse"></i> Warehouses To</label>
                <?php
                $whs = $this->site->getWarehouses(['active' => 1]);
                ...
                ?>
                <?= form_multiselect('warehouses_to[]', $bl, ($warehouses ?? ''), 'class="select2" id="warehousesTo" data-placeholder="Select warehouse to" style="width:100%;"'); ?>
              </div>
            </div>
          </div>
          <div class="row">
...
<script>
  $(document).ready(function() {
    ...
    $('#btn_filter').click(function() {
      ...
      let warehousesFrom = $('#warehousesFrom').val();
      let warehousesTo = $('#warehousesTo').val();
      let q = '?';
      ...
      if (warehousesFrom) {
        for (let x in warehousesFrom) {
          q += '&warehouses_from[]=' + warehousesFrom[x];
        }
      }
      if (warehousesTo) {
        for (let x in warehousesTo) {
          q += '&warehouses_to[]=' + warehousesTo[x];
        }
      }
