...
<div class="modal-content">
  <div class="modal-header">
    ...
    <h4 class="modal-title text-center" id="myModalLabel"><i class="fad fa-clock"></i> Edit Schedule</h4>
  </div>
...
<script>
  $(document).ready(function() {
    let sun = JSON.parse('<?= json_encode(explode('-', $schedule->hour_sun)) ?>');
    let mon = JSON.parse('<?= json_encode(explode('-', $schedule->hour_mon)) ?>');
    let tue = JSON.parse('<?= json_encode(explode('-', $schedule->hour_tue)) ?>');
    let wed = JSON.parse('<?= json_encode(explode('-', $schedule->hour_wed)) ?>');
    let thu = JSON.parse('<?= json_encode(explode('-', $schedule->hour_thu)) ?>');
    let fri = JSON.parse('<?= json_encode(explode('-', $schedule->hour_fri)) ?>');
    let sat = JSON.parse('<?= json_encode(explode('-', $schedule->hour_sat)) ?>');

    let billerIds = JSON.parse('<?= (!empty($schedule->billers) ? json_encode(explode(',', $schedule->billers)) : '[]') ?>');

    $('#biller').val(billerIds).trigger('change');
    ...
    $('#submit').click(function() {
      ...
      $.ajax({
        ...
        url: site.base_url + 'schedule/edit/<?= $schedule->id ?>'
      });
    });
  });
</script>