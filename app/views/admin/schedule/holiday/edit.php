...
<div class="modal-content">
  <div class="modal-header">
    ...
    <h4 class="modal-title text-center" id="myModalLabel"><i class="fad fa-clock"></i> Edit Holiday</h4>
  </div>
...
<script>
  $(document).ready(function() {
    let billerIds = JSON.parse('<?= (!empty($holiday->billers) ? json_encode(explode(',', $holiday->billers)) : '[]') ?>');

    $('#biller').val(billerIds).trigger('change');
    $('#description').val('<?= htmlDecode($holiday->description) ?>');
    $('#holiday_from').val('<?= $holiday->start_date ?>');
    $('#holiday_to').val('<?= $holiday->end_date ?>');
    $('#working_from').val('<?= $holiday->start_work ?>');
    $('#working_to').val('<?= $holiday->end_work ?>');
    ...
    $('#submit').click(function() {
      ...
      $.ajax({
        ...
        url: site.base_url + 'schedule/holiday/edit/<?= $holiday->id ?>'
      });
    });
  });
</script>