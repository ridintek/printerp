$(document).ready(function () {
  ...
  if (qawarehouse = localStorage.getItem('qawarehouse')) {
    $('#qawarehouse').val(qawarehouse).trigger('change');
  }
...
function loadItems () {
  ...
  if (localStorage.getItem('qaitems')) {
    ...
    $.each(sortedItems, function () {
      ...
      var source_qty = item.row.source_qty;

      var opt = $("<select name=\"variant\[\]\" class=\"form-control select rvariant\" />");
      ...
      tr_html += '<td class="text-right">' + formatQuantity(source_qty, 2) + '</td>';
    ...
    var col = 1;
    var tfoot = '<tr id="tfoot" class="tfoot active"><th colspan="'+col+'">Total</th><th class="text-center">' + formatQuantity4(parseFloat(count) - 1) + '</th>';
    tfoot += '<th></th>';
    ...
    $('select.select').select2({minimumResultsForSearch: 20});
